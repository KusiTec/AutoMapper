...
namespace AutoMapper.Internal
{
    public static class TypeExtensions
    {
        ...
        private static IEnumerable<Type> BaseClassesAndInterfaces(this Type type) => type.GetTypeInheritance().Concat(type.GetInterfaces());

        const BindingFlags Flags = BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.DeclaredOnly;

        public static PropertyInfo GetInheritedProperty(this Type type, string name) =>
            type.BaseClassesAndInterfaces().Select(t => t.GetProperty(name, Flags)).FirstOrDefault(p => p != null);

        public static FieldInfo GetInheritedField(this Type type, string name) =>
            type.BaseClassesAndInterfaces().Select(t => t.GetField(name, Flags)).FirstOrDefault(f => f != null);

        public static MethodInfo GetInheritedMethod(this Type type, string name) =>
            type.BaseClassesAndInterfaces().Select(t => t.GetMethod(name, Flags)).FirstOrDefault(m => m != null)
